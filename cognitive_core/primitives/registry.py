...
PRIMITIVE_CONFIGS: dict[str, dict[str, Any]] = {
    ...
    "deliberate": {
        ...
        "prompt_file": "deliberate.txt",
        "schema": DeliberateOutput,
    },
    "govern": {
    ...
    "reflect": {
        ...
        "optional_params": ["domain_index", "additional_instructions"],
        "defaults": {
            "domain_index": "No domain index provided — reflect on accumulated state only.",
            "additional_instructions": "",
            "context": "No additional context provided.",
        },
        "prompt_file": "reflect.txt",
...
def render_prompt(primitive_name: str, params: dict[str, str]) -> str:
    ...
    # Render template
    # Pre-escape literal braces (e.g., JSON examples in prompts) before str.format()
    # Strategy: protect actual template slots {word}, escape everything else.
    import re
    slots = set(re.findall(r'\{(\w+)\}', template))
    protected = template
    for slot in slots:
        protected = protected.replace('{' + slot + '}', f'__SLOT_{slot}__')
    protected = protected.replace('{', '{{').replace('}', '}}')
    for slot in slots:
        protected = protected.replace(f'__SLOT_{slot}__', '{' + slot + '}')

    try:
        rendered = protected.format(**merged)
