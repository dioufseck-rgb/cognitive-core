...
Usage:
    from cognitive_core.engine.governance import get_governance, GovernancePipeline
...
def _extract_text(content: object) -> str:
    """
    Normalise LangChain model response content to a plain string.

    LangChain responses vary by provider and version:
      - str        -- common case (most models)
      - list[dict] -- newer Gemini/multimodal: [{"type": "text", "text": "..."}]
      - list[str]  -- some wrappers
      - dict       -- single content block
      - other      -- fall back to str(), never raise

    Extracts all text content regardless of shape so callers always get a str.
    """
...
class GovernancePipeline:
    ...
    def initialize(self):
        ...
        # ── Input Guardrails ──
        try:
            from cognitive_core.engine.guardrails import InputGuardrail
        ...
        # ── PII Redaction ──
        try:
            from cognitive_core.engine.pii import PiiRedactor
        ...
        # ── Semantic Cache ──
        try:
            from cognitive_core.engine.semantic_cache import SemanticCache
        ...
        # ── Cost Tracker ──
        try:
            from cognitive_core.engine.cost import CostTracker
        ...
        # ── Circuit Breaker ──
        try:
            from cognitive_core.engine.logic_breaker import get_logic_breaker
        ...
        # ── Kill Switches ──
        try:
            from cognitive_core.engine.kill_switch import KillSwitchManager
        ...
        # ── Shadow Mode ──
        try:
            from cognitive_core.engine.shadow import ShadowMode
        ...
        # ── Compensation Ledger ──
        try:
            from cognitive_core.engine.compensation import CompensationLedger
        ...
        # ── Webhook Notifier ──
        try:
            from cognitive_core.engine.webhooks import WebhookNotifier, WebhookConfig
        ...
        # ── Replay Manager ──
        try:
            from cognitive_core.engine.replay import ReplayManager
        ...
        # ── HITL Routing ──
        try:
            from cognitive_core.engine.hitl_routing import RoutingManager
        ...
        # ── HITL State Machine ──
        try:
            from cognitive_core.engine.hitl_state import HITLStateMachine
    ...
    def protected_llm_call(
        ...
        _messages = [HumanMessage(content=working_prompt)]
        _policy = get_retry_policy(provider)

        def _rate_limited_invoke(llm_=llm, msgs=_messages):
            """Single invoke call, wrapped in rate limiter if configured."""
            if limiter:
                try:
                    with limiter.acquire(timeout=30):
                        return llm_.invoke(msgs)
                except Exception as _e:
                    logger.warning("Rate limit acquire failed for %s/%s: %s",
                                   provider, step_name, _e)
                    return llm_.invoke(msgs)
            return llm_.invoke(msgs)

        t0 = time.time()
        _retry_result = invoke_with_retry(
            llm=llm,
            messages=_messages,
            policy=_policy,
            step_name=step_name,
            # parse_fn intentionally omitted: protected_llm_call is used for
            # both JSON and non-JSON responses. JSON validation and parse-failure
            # retries are the responsibility of the caller (nodes.py extract_json).
            invoke_fn=_rate_limited_invoke,
        )
        elapsed_ms = (time.time() - t0) * 1000
        result.latency_ms = elapsed_ms

        # _retry_result.content is already a normalised string:
        # _extract_text() is applied inside invoke_with_retry on response.content.
        raw = _retry_result.content
        ...
        if self._cost_tracker:
            try:
                # Extract token counts from RetryResult metadata if available.
                # Token usage was passed through from the raw response in invoke_with_retry.
                input_tokens = getattr(_retry_result, "input_tokens", 0) or 0
                output_tokens = getattr(_retry_result, "output_tokens", 0) or 0
    ...
    def check_start_gates(
        ...
        if enforced and model_id:
            try:
                from cognitive_core.engine.eval_gate import EvalGate, EvalGateNotPassedError
                _gate = EvalGate()
    ...
    def capture_spec_manifest(
        ...
        try:
            from cognitive_core.engine.spec_lock import create_manifest, is_spec_lock_enabled
    ...
    def resolve_tier(self, declared_tier: str, domain: str) -> tuple[str, str]:
        ...
        self.initialize()
        try:
            from cognitive_core.engine.tier import resolve_effective_tier
    ...
    # Called from engine/nodes.py → create_govern_node()
    # ═══════════════════════════════════════════════════════════════
    ...
    def should_skip_act(self, step_name: str) -> bool:
        ...
        if self._shadow_mode:
            try:
                skip = self._shadow_mode.should_skip_act("govern")
                self._record_proof("govern.shadow_check",
        ...
        return {
            "governance_proof": {
                ...
                "summary": {
                    ...
                    "govern_checks": len(categories.get("govern", [])),
