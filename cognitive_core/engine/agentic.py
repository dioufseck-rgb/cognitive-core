...
from cognitive_core.primitives.registry import render_prompt, get_schema_class, PRIMITIVE_CONFIGS
...
def compose_agentic_workflow(
        ...
        if steps_completed:
            ...
            for s in steps_completed:
                ...
                if s["primitive"] == "classify":
                    ...
                    options = out.get("options_considered", [])
                    summary += f"→ {len(options)} options, action: {out.get('recommended_action', 'none')}"
                steps_summary_parts.append(summary)
    ...
    def primitive_executor(state: WorkflowState) -> dict:
        ...
        from cognitive_core.engine.state import resolve_param
        resolved_params = {}
