...
def create_case_registry(case_data: dict[str, Any], fixtures_dir: str = "") -> ToolRegistry:
    """
    ...
    2. Fixture-based: case_data has only identity fields (claim_id, etc.)
       and tool data is loaded from <fixtures_dir>/*_tools.json

    fixtures_dir: Directory to scan for fixture tool files.
        Defaults to "cases/fixtures" for backward compatibility.
        The coordinator passes its configured case_dir.
    ...
    if not has_embedded_tools:
        # Load from fixtures
        tool_data = _load_fixtures_for_case(case_data, fixtures_dir)
    else:
...
def _load_fixtures_for_case(case_data: dict[str, Any], extra_dir: str = "") -> dict[str, Any]:
    """
    Load tool fixtures for a case.

    Scans directories in order for *_tools.json files matching case_id:
    1. extra_dir (if provided — the coordinator's case_dir)
    2. extra_dir/fixtures (subdirectory convention)
    3. cases/fixtures (framework default)
    """
    from pathlib import Path

    lookup_id = case_data.get("case_id") or case_data.get("claim_id", "")

    scan_dirs = []
    if extra_dir:
        p = Path(extra_dir)
        scan_dirs.append(p)
        if (p / "fixtures").exists():
            scan_dirs.append(p / "fixtures")
    default = Path("cases/fixtures")
    if default.exists():
    ...
    for d in scan_dirs:
        if not d.exists():
            continue
        for f in sorted(d.glob("*_tools.json")):
            try:
                with open(f) as fp:
                    data = json.load(fp)
                file_id = data.get("_case_id") or data.get("_claim_id", "")
                if file_id == lookup_id:
                    return data
