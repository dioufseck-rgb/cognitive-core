...
class RetryResult:
    ...
    content: str                      # raw LLM response text (normalised string)
    ...
    input_tokens: int = 0             # token counts from provider metadata (best-effort)
    output_tokens: int = 0
...
def invoke_with_retry(
    ...
    for attempt in range(policy.max_attempts):
        ...
        try:
            ...
            _call = invoke_fn if invoke_fn is not None else (lambda: llm.invoke(messages))
            ...
            # Normalise response.content — providers vary in what they return.
            try:
            ...
            # Extract token counts from response metadata (best-effort, provider-varies)
            _input_tok = 0
            _output_tok = 0
            try:
                _usage = getattr(response, "response_metadata", {}) or {}
                if isinstance(_usage, dict):
                    _usage = _usage.get("usage", _usage)
                if isinstance(_usage, dict):
                    _input_tok = int(_usage.get("input_tokens", _usage.get("prompt_tokens", 0)) or 0)
                    _output_tok = int(_usage.get("output_tokens", _usage.get("completion_tokens", 0)) or 0)
            except Exception:
                pass
