...
def compute_mechanical(
    ...
    if primitive == "retrieve":
        ...
        # Citation rate: evidence_used count as proxy for citation density.
        # Only computed when the LLM actually populated evidence_used —
        # an empty list is ambiguous (genuinely no evidence cited, or prompt
        # non-compliance) so treat it as no measurement rather than a low score.
        # compute_overall falls back to 1.0 when mechanical_scores is empty,
        # which is the correct neutral signal when we have no measurement.
        ...
        # If evidence_used is empty: leave citation_rate = None (no measurement)
