...
from cognitive_core.engine.state import WorkflowState, get_step_output, get_loop_count
from cognitive_core.engine.tools import ToolRegistry, create_case_registry
from cognitive_core.engine.actions import ActionRegistry
...
if TYPE_CHECKING:
    ...
    from cognitive_core.primitives.registry import validate_use_case_step
    from cognitive_core.engine.nodes import (
...
def _import_nodes():
    ...
    from cognitive_core.engine.nodes import (  # noqa: F811
        create_node, create_retrieve_node, create_govern_node,
        create_agent_router,
    )
    from cognitive_core.engine.trace import get_trace  # noqa: F811
    return create_node, create_retrieve_node, create_govern_node, create_agent_router, get_trace


def _import_validate():
    """Lazy import of registry validation."""
    from cognitive_core.primitives.registry import validate_use_case_step  # noqa: F811
...
def validate_use_case(config: dict[str, Any]) -> list[str]:
    ...
    for step in config["steps"]:
        ...
        # Validate loop_fallback when max_loops is set
        if step.get("max_loops") is not None and "loop_fallback" not in step:
            import warnings
            warnings.warn(
                f"Step '{step['name']}' has max_loops={step['max_loops']} but no "
                f"loop_fallback. Graph will terminate (__end__) when max_loops is "
                f"reached. Set loop_fallback to a step name to continue instead.",
                stacklevel=2,
            )
    return errors
...
def compose_workflow(
    ...
    create_node, create_retrieve_node, create_govern_node, create_agent_router, get_trace = _import_nodes()
...
def compose_subgraph(
    ...
    """
    StateGraph, END = _import_langgraph()
    create_node, create_retrieve_node, create_govern_node, _, _ = _import_nodes()
    from cognitive_core.engine.resume import collect_reachable_steps, clamp_transitions
