...
This file is based on copied source code from the PythonPDEVS minimal
simulation kernel (Van Tendeloo & Vangheluwe, 2014), extended with
real-time execution and external event injection for use as Cognitive
Core's workflow execution engine.
...
Original source:
    ...
    Copyright 2014 MSDL — Licensed under the Apache License, Version 2.0

Extensions added for Cognitive Core (beyond the original kernel):
    ...
    - stop() method for clean simulator shutdown
...
# Original PythonPDEVS source:
#   Copyright 2014 Modelling, Simulation and Design Lab (MSDL)
#   McGill University and the University of Antwerp
#   Licensed under the Apache License, Version 2.0
#
# Cognitive Core extensions:
#   Copyright 2026 Mamadou Seck
#   Licensed under the Apache License, Version 2.0
...
class Simulator:
    ...
    def simulate(self):
        ...
        while not self._termination_fn(tn, self.original_model):
            ...
            if tn[0] == INFINITY:
                # Check termination before waiting — the condition may already
                # be satisfied (e.g. all steps passive after the last step
                # fires intTransition done→passive). Without this check the
                # real-time loop waits _inject_wait_timeout before discovering
                # the simulation is complete.
                if self._termination_fn(tn, self.original_model):
                    break
                if self._real_time:
                    ...
                    if not has_pending:
                        ...
                        if not woken:
                            break  # No inject arrived — terminate
                    tn = scheduler.readFirst()
                    continue
                else:
                    break
