...
class PolicyEngine:
    ...
    def evaluate_delegations(
        ...
        for policy in self.delegation_policies:
            ...
            decisions.append(DelegationDecision(
                ...
                governance_tier=policy.governance_tier,
            ))
    ...
    def validate_work_order_inputs(
        ...
        if not contract_name:
            return []  # No contract specified — skip validation
    ...
    def _single_condition_matches(
        ...
        """
        Evaluate a single condition using selectors.

        Primitive-type selectors: any_<prim>, last_<prim>, all_<prim>
        Step-name selectors: step:<name> (matches a specific step by name)
        Special: final_output (last step regardless of type)
        """
        selector = cond.selector

        # Step-name selector: step:<step_name>
        if selector.startswith("step:"):
            step_name = selector[5:]
            matching = [s for s in steps if s.get("step_name") == step_name]
            if not matching:
                return False
            return self._field_matches(matching[-1].get("output", {}), cond)
    ...
    def _resolve_source_path(
        ...
        """
        Resolve a source path against workflow output.

        Supports:
          input.field                    → case_input field
          last_<prim>.field              → last step of primitive type
          any_<prim>.field               → first match across primitive type
          step:<step_name>.field         → specific step by name
        """
        parts = path.split(".", 1)
        selector = parts[0]
...
def load_policy_engine(config: dict[str, Any]) -> PolicyEngine:
    ...
    for d in config.get("delegations", []):
        ...
        delegations.append(DelegationPolicy(
            ...
            governance_tier=d.get("governance_tier", ""),
        ))
