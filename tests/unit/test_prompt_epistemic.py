...
class TestRenderPromptBraceEscaping:
    """Regression tests for unescaped brace bug in render_prompt.

    JSON examples in prompt templates contain literal braces like
    {"claim_id": "CLM-001"} which must NOT be treated as format slots.
    """

    def test_render_prompt_with_json_braces_in_template(self):
        """render_prompt must not raise on prompt templates with JSON examples."""
        from cognitive_core.primitives.registry import render_prompt
        # Every primitive prompt contains JSON examples — use generate as the
        # canonical case from the bug report.
        rendered = render_prompt(
            "generate",
            {
                "requirements": "Write a determination letter.",
                "format": "structured text",
                "constraints": "Be concise.",
                "context": "Patient: test",
                "input": "Appeal case data: patient X, ACDF C5-C6.",
            },
        )
        assert isinstance(rendered, str)
        assert len(rendered) > 0

    def test_render_prompt_preserves_literal_braces_in_output(self):
        """Literal JSON braces in the template survive into the rendered output."""
        from cognitive_core.primitives.registry import render_prompt
        rendered = render_prompt(
            "classify",
            {
                "categories": "A, B",
                "criteria": "Choose A if X, B if Y.",
                "context": "test context",
                "input": "Sample input to classify.",
            },
        )
        # The rendered prompt should contain at least one { from JSON examples
        assert "{" in rendered or "}" in rendered or len(rendered) > 100
