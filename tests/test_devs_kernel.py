...
# ─── Test 7: Termination condition fires promptly when all models passive ─────

def test_termination_on_all_passive():
    """
    Regression test for the 20-minute hang bug.

    When all components reach passive state (tn == INFINITY), the real-time
    simulator must check the termination condition immediately — not wait for
    an inject that never arrives.

    Previously: the simulator entered _inject_event.wait(timeout=1200) before
    checking the termination condition, causing every workflow to hang for up
    to 20 minutes after the last step completed.

    Fixed: termination condition is now checked before entering the inject wait.
    """
    class PassiveAfterOneModel(AtomicDEVS):
        """Fires once immediately, then goes passive."""
        def __init__(self):
            AtomicDEVS.__init__(self, "one_shot")
            self.state = {"phase": "active"}

        def timeAdvance(self):
            return 0.0 if self.state["phase"] == "active" else INFINITY
    ...
    model = PassiveAfterOneModel()
    sim = Simulator(model)
    sim.setRealTime(True)

    # Termination: when model is passive
    sim.setTerminationCondition(
        lambda tn, m: model.state["phase"] == "passive"
    )
    ...
    assert model.state["phase"] == "passive"
    assert elapsed < 2.0, (
        f"Simulation took {elapsed:.1f}s — expected < 2s. "
        f"Termination condition was not checked before inject wait."
    )
    print(f"✓ test_termination_on_all_passive ({elapsed:.2f}s)")
